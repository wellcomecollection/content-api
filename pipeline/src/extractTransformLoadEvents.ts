...
import { EventPrismicDocument } from "./types/prismic";
...
export const isParentDocument = (
  doc: prismic.PrismicDocument
): doc is EventPrismicDocument => ["events"].includes(doc.type);

export const ETLEvents = async (clients: Clients, event: WindowEvent) => {
  ...
  const remainingParentDocuments = otherDocuments.pipe(
    ...
    getDocumentsByID<EventPrismicDocument>(clients.prismic, {
      graphQuery: eventDocumentsQuery,
    }),
    tap(() => {
