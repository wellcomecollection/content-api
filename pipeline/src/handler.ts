...
// TODO type with Handler for Lambda, couldn't use it locally
export const createHandler = async (clients: Clients) => {
  try {
    // Fetch all articles and webcomics from Prismic
    const searchResponse = await getPrismicDocuments<ArticlePrismicDocument[]>({
