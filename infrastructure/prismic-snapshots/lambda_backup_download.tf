...
resource "null_resource" "lambda_backup_download_build" {
  triggers = {
    ...
    lambda_code = filemd5("${path.module}/lambda/${local.lambda_backup_download_name}.js")
  ...
  provisioner "local-exec" {
    command = "${path.module}/scripts/build-lambda.sh ${local.lambda_backup_download_name} ${path.module}/prismic_backup_download_lambda.zip"
  }
}
