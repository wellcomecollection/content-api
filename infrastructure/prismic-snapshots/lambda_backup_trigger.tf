...
resource "null_resource" "lambda_backup_trigger_build" {
  triggers = {
    ...
    lambda_code = filemd5("${path.module}/lambda/${local.lambda_backup_trigger_name}.js")
    # Rebuild when the build script changes
    build_script = filemd5("${path.module}/scripts/build-lambda.sh")
  }

  provisioner "local-exec" {
    command = "${path.module}/scripts/build-lambda.sh ${local.lambda_backup_trigger_name} ${path.module}/prismic_backup_trigger_lambda.zip"
  }
}
